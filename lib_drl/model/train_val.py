...
class SolverWrapper(object):
    ...
    def construct_graph(self, sess):
        ...
        # 以至于能够得到相同的结果，那么久需要到设置随机函数的seed 参数，对应的变量可以跨session
        # 生成相同的随机数：
    ...
    def train_model(self, sess, max_iters):
        ...
        # 两个部分的训练模型训练数据 #6934 照片數與實際不同在於它會翻轉(flipped)
        self.data_layer_val = RoIDataLayer(self.valroidb, cfg.NBR_CLASSES, True)  # 867

        # Construct the computation graph corresponding to the original Faster R-CNN
        # architecture first
        lr_det_op, train_op = self.construct_graph(sess)  # network.py在此處工作_rois_seq_batched

        # We will handle the snapshots ourselves
        # 使用tf.train.Saver 這個物件 (object) 來儲存模型．它可以幫助我們簡單的把變量 (variables) 儲存成檔案，
        # 可儲存ckpt檔
        # snapshots 可能指的是每次儲存的那個時刻，就像快照一樣
        ...
        # Sample first beta
        # Fixation penalty(ies) used in training # 0.05
        beta = cfg.LRP_HAI_TRAIN.BETA

        # Setup LRP-HAI timers # t = Timer(10.0, hello)十秒之後輸出hello
        ...
        while iter < max_iters:
            ...
            # snapshot（快照的作用主要是能够进行在线数据备份与恢复。
            # 当存储设备发生应用故障或者文件损坏时可以进行快速的数据恢复，
            # 将数据恢复某个可用的时间点的状态。）
...
def train_net(network, imdb, roidb, valroidb, output_dir, tb_dir,
    ...
    # 可以设置tf.ConfigProto()中参数allow_soft_placement = True，
    ...
    with tf.Session(config=tfconfig) as sess:
        ...
        # train # max_iters 110000
        #print([n.name for n in sw.as_graph_def().node])
