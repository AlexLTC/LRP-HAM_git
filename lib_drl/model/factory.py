...
def update_obs_vol(roi_obs_vol, t, height, width,
    ...
    # at time 1,2,..., t-1 # true or false
    already_obs_rois_idxs_orig = roi_obs_vol > 0
    already_obs_rois_orig = roi_obs_vol[already_obs_rois_idxs_orig]

    # Update RoI volume
    # 將照片中心座標'fix_h,fix_w'及方框長度'obs_rect_h及obs_rect_w',換成 'h_start, w_start, h_end, w_end'
...
def run_LRP_HAI(sess, net, blob, timers, mode, beta, im_idx=None,
    ...
    # Save visualization (if desired) #儲存demo照片demo_drl.py
...
def save_visualization(im_blob, im_shape, im_idx, obs_canvas, cls_probs,
    ...
    for i in range(obs_canvas.shape[2]):
        ...
        for j in range(len(names_and_coords)):
            ...
            """
	        # alex add for ouput img's (x,y,w,h)
            with open('/home/user/Desktop/alex/real_demo_test', 'a') as f :
                x1_inf = 'x1 :' + str(cls_det[0]) + '\n'
                x2_inf = 'x2 :' + str(cls_det[2]) + '\n'
                y1_inf = 'y1 :' + str(cls_det[1]) + '\n'
                y2_inf = 'y2 :' + str(cls_det[3]) + '\n'
                w_inf = 'w :' + str(cls_det[2] - cls_det[0]) + '\n'
                h_inf = 'h :' + str(cls_det[3] - cls_det[1]) + '\n'
                f.write(x1_inf)
                f.write(x2_inf)
                f.write(y1_inf)
                f.write(y2_inf)
    ...
    if ~show_all_steps:
        ...
        #plt.savefig('./demo_image/1/'+im_name, transparent=True, bbox_inches='tight', pad_inches=0.0)
        plt.savefig('./demo_result/'+im_name, transparent=True, bbox_inches='tight', pad_inches=0.0)
