...
# from nets.model_neurips import DQN, get_tf_output
...
class Network(object):
    ...
    def _add_gt_image(self):
        ...
        # 變 ori pic
        _im_info = self._im_info
        # _im_info[0] = self._im_info[0] / self._im_info[2]# x
        # _im_info[1] = self._im_info[1] / self._im_info[3]
        resized = tf.image.resize_bilinear(image, tf.to_int32(self._im_info[:2] / self._im_info[2]))
        # resized = tf.image.resize_bilinear(image, tf.to_int32(_im_info))
    ...
    def _proposal_target_layer_wo_scores(self, rois, name):  # rois = 'self._rois_seq', name = 'rois_seq_batched'
        with tf.variable_scope(name) as scope:
            # tf.py_function(func, inp, Tout, name=None)
            # proposal_target_layer_wo_scores(rpn_rois, gt_boxes, _num_classes)
    ...
    def train_LRP_HAI(self, sess, lr_rl, sc, stats, cur_iter, writer):
        ...
        if cfg.LRP_HAI_TRAIN.USE_BL:
            ...
            for idx in range(cfg.LRP_HAI.MAX_ITER_TRAJ):
                if len(self._bl_total[idx]) > 0:
                    bl_means_total[idx] = np.mean(self._bl_total[idx])# 取平均
                    bl_stds_total[idx] = np.std(self._bl_total[idx])# 標準差
        ...
        for idx in range(len(self._ep_batch['x'])):
            ...
            # ep_batch['total_return'] 共有50個內容物，每個size均不同

            # rew = (rew - bl_mean) / std ---- normalize to mean 0, std 1
            if cfg.LRP_HAI_TRAIN.USE_BL:
            ...
            feed_dict_grad_comp \
                = {self._rl_in: self._ep_batch['x'][idx],
                   ...
                   self._done_labels: self._ep_batch['done'][idx],# TR
                   self._fix_labels: self._ep_batch['fix'][idx],# P,position
            ...
            curr_batch_ce_fix_rew_prod += ce_fix_rew_prod   # P

            # Accumulate gradients to buffer
            for ix, grad in enumerate(new_grads):   # 跑17次,ix=16,grad.shape=[625,1]
    ...
    def _build_network(self, is_training=True):
        ...
        rois_in = tf.cond(tf.equal(self._cond_switch_roi, 0),
                          ...
                          lambda: self._net_rois_seq())  # choose this
        ...
        elif cfg.POOLING_MODE == 'roi_align':  # choose this
    ...
    def HAI(self, is_training, initializer, name='HAI'):
        ...
        self.wk_kernal = self._make_var('wk_kernal', [3, 3, dims_base, 300],
                                        initializer, is_training)
        self.uk_kernal = self._make_var('uk_kernal', [3, 3, 300, 300],
                                        initializer, is_training)
        self.wr_kernal = self._make_var('wr_kernal', [3, 3, dims_base, 300],
                                        initializer, is_training)
        self.ur_kernal = self._make_var('ur_kernal', [3, 3, 300, 300],
                                        initializer, is_training)
        self.wz_kernal = self._make_var('wz_kernal', [3, 3, dims_base, 300],
                                        initializer, is_training)
        self.uz_kernal = self._make_var('uz_kernal', [3, 3, 300, 300],
                                        initializer, is_training)
        self.wh_kernal = self._make_var('wh_kernal', [3, 3, dims_base, 300],
                                        initializer, is_training)
        self.uh_kernal = self._make_var('uh_kernal', [3, 3, 300, 300],
                                        initializer, is_training)
        self.a = self._make_var('a', [1], initializer, is_training)
        self.g = self._make_var('g', [1], initializer, is_training)
        self.beta = self._make_var('beta', [1], initializer, is_training)
        self.att_matmul = self._make_var('att_weights', [3, 3, 300, dims_base],
                                        initializer, is_training)
        self.att_matmul_base = self._make_var('att_matmul_base', [dims_base], initializer, is_training)
        self.alpha_prev = tf.zeros([1, in_shape[1]*in_shape[2], dims_base])
        ...
        i, done_logits_all, fix_logits_all, done_prob, h, fix_prob_map \
            ...
            # tf.while_loop( cond, body,loop_vars,shape_invariants=None,parallel_iterations=10, back_prop=True, swap_ memory=False, maximum_ iterations=None, name=None)
    ...
    def HAI_rollout(self, i, done_logits_all, fix_logits_all, done_prob, h, fix_prob_map):
        ...
        # Tensor("while/Slice:0", shape=(1, ?, ?, 256), dtype=float32)


        #tf.slice tf.slice(input_, begin, size, name = None)
        # 这个函数的作用是从输入数据input中提取出一块切片
        # 切片的尺寸是size，切片的开始位置是begin。
        input_shape = tf.shape(rl_base)
        height = input_shape[1]
        width = input_shape[2]
        # rl_base = tf.image.resize_images(rl_base, (38, 38))
        # h = tf.image.resize_images(h, (38, 38))


        # wa = self.conv(rl_base, self.wa_kernal)
        # wh = self.conv(h, self.wh_kernal)
        # wa = tf.matmul(rl_base, self.wa_kernal)
        # wh = tf.matmul(h, self.wh_kernal)


        # w_a_h = wa + wh
        k = tf.tanh(self.a*(self.conv(rl_base, self.wk_kernal)+self.conv(h, self.uk_kernal)))
        # wx+b change here
        # st = tf.matmul(k, self.att_matmul) + self.att_matmul_base
        st = self.conv(k, self.att_matmul) + self.att_matmul_base
        st = tf.reshape(st, [1, -1, dims_base])

        at = tf.nn.log_softmax(st, dim=1)
        alpha_ = at * self.g + self.alpha_prev * (1 - self.g)

        if alpha_.shape == self.alpha_prev.shape:
            self.alpha_prev.data = alpha_

        A_arg = tf.nn.log_softmax(alpha_*self.beta, dim=1)
        # Yt = tf.multiply(Yt_b, rl_base)

        r = tf.sigmoid(self.conv(rl_base, self.wr_kernal)+self.conv(h, self.ur_kernal))
        z = tf.sigmoid(self.conv(rl_base, self.wz_kernal)+self.conv(h, self.uz_kernal))

        h_ = tf.tanh((r * self.conv(h, self.uh_kernal)) + self.conv(rl_base, self.wh_kernal)) * (1 - z)
        h_p = h * z
        ht = h_p + h_

        # A_arg = tf.reshape(A_arg, [1, -1, dims_base])
        # A_arg_t = tf.transpose(A_arg, [0, 1, 3, 2])
        Yt_b = tf.reduce_sum(A_arg, -1, keep_dims=True)

        h = ht
        # attention_sum = Yt_b
        attention_sum = tf.reshape(Yt_b, [1, height, width, 1])
        ...
        # argmax(.) 找出region localization : attention_sum -> fix_prob_map
        ...
        print('done_prob', done_prob.shape)
        print('fix_prob_map', fix_prob_map.shape)
    ...
    def create_architecture(self, mode, num_classes, tag=None,
        ...
        training = mode == 'TRAIN'
        # if mode=TRAIN ,training=True

        # handle most of the regularizers here
        weights_regularizer \
            = tf.contrib.layers.l2_regularizer(cfg.TRAIN.WEIGHT_DECAY)  # 0.0001
        # Regularization 防止 overfitting 類似降維的效果，J = J'+S/2*||w||2，s=0.0001

        if cfg.TRAIN.BIAS_DECAY:  # bias_decay = false
            biases_regularizer = weights_regularizer
        else:
            biases_regularizer = tf.no_regularizer  # Use this function to prevent regularization of variables.
        ...
        with arg_scope([slim.conv2d, slim.conv2d_in_plane,
            ...
            self._build_network(training)  # training=True
    ...
    # 'cross_entropy''loss_box''total_loss''net_conv''cls_score_seq''cls_pred_seq''cls_prob_seq''bbox_pred_seq'
