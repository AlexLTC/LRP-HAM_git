...
class SolverWrapper(object):
    ...
    def __init__(self, sess, network, imdb, roidb, valroidb, output_dir, tbdir, pretrained_model=None, logger=None):
        self.net = network
        self.imdb = imdb
        self.roidb = roidb
    ...
    def get_variables_in_checkpoint_file(self, file_name):
        try:
            ...
            if "corrupted compressed block contents" in str(e):
                self.logger.info("It's likely that your checkpoint file has been compressed "
                                 "with SNAPPY.")
    ...
    def initialize(self, sess):
        ...
        var_keep_dic = self.get_variables_in_checkpoint_file(self.pretrained_model)  # para's name from early ckpt
    ...
    def train_model(self, sess, max_iters):
        ...
        while iter < max_iters + 1:
            ...
            # 'need_backprop': array([[1]], dtype=int32), 
            # 'dc_label': array([[1.]], dtype=float32), 
            # 'gt_boxes': array([[371.25, 196.25, 537.5 , 358.75,   1.]], dtype=float32), 
            # 'im_info': array([600.  , 800.  ,   1.25], dtype=float32)}])
            ...
            if iter == 1 or now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
                ...
                is_target, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, da_conv_loss, total_loss, summary = \
                ...
                is_target, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, da_conv_loss, total_loss = \
...
def train_net(network, imdb, roidb, valroidb, output_dir, tb_dir,
    ...
    logger = logging.getLogger("faster-rcnn.train_net")

    with tf.Session(config=tfconfig) as sess:
        sw = SolverWrapper(sess, network, imdb, roidb, valroidb, output_dir, tb_dir,
                           pretrained_model=pretrained_model, logger=logger)
        logger.info('Solving...')
