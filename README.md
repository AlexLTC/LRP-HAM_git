...
1. Clone the repository
  ```Shell
  git clone https://github.com/aleksispi/LRP-HAI-tf.git
  ```
...
The corresponding results when using our LRP-HAI detector with exploration penalty 0.05 during inference (models trained over different exploration penalties, as described in Section 5.1.2 in the paper) and posterior class-probability adjustments (Section 4.2 in our paper):
...
| RPN              | 76.5           | 74.2           |
| LRP-HAI          | 77.5           | 74.9           |
| LRP-HAI (np)     | 77.2           | 74.6           |
| LRP-HAI (12-fix) | 77.6           | 75.0           |
...
**Note**:
  ...
  - See the code for any deviations from the [CVPR 2018 paper](http://openaccess.thecvf.com/content_cvpr_2018/papers/Pirinen_Deep_Reinforcement_Learning_CVPR_2018_paper.pdf). Some important changes post-CVPR:
    ...
    - The LRP-HAI models are now much more fast to train than how it was done in the original paper (c.f. Section 5.2). Specifically, instead of sampling 50 search trajectories per image to estimate the policy gradient, we now run 50 search trajectories on 50 *different* images. This reduces training time by 5-10 times, yet we get results in the same ball park.

### Pretrained models
All pretrained models (both Faster R-CNN baseline and our LRP-HAI models) for the numbers reported above in *Detection Performance* is available on google drive:
- LRP-HAI trained on VOC 2007+2012 trainval: https://drive.google.com/open?id=1iK8fxp6no9g_-eZ2b2G0FRKV0cfUX53r
- LRP-HAI trained on VOC 2007+2012 trainval + 2007 test: https://drive.google.com/open?id=1rNwmXLz9VCdK3s6dFqBH3rqpuVFtMLK7
...
3. The main script to launch training is `experiments/scripts/train_LRP_HAI.sh`. Setup `SAVE_PATH` and `WEIGHT_PATH` appropriately, and run the command
`./experiments/scripts/train_LRP_HAI.sh 0 pascal_voc_0712 1 20000 0 110000` to start training on VOC 2007+2012 trainval on GPU-id 0 for a total of 110k iterations (see code for more details). This will yield a LRP-HAI model trained over two exploration penalties, enabling setting the speed-accuracy trade-off at test time. See also `experiments/cfgs/LRP-HAI-vgg16.yml` for some settings.
4. Once the above model has finished training in step 3, it is also possible to train the posterior class-probability history module (c.f. Section 4.2 in our paper). To do this, first make sure that the `WEIGHTS_PATH` variable in `train_LRP_HAI.sh` points to your LRP-HAI model weights obtained in step 3 above. Then run `./experiments/scripts/train_LRP_HAI.sh 0 pascal_voc_0712 1 0 1 110000` to train the posterior class-probability adjustment module for 110k iterations.

### Testing LRP-HAI
1. Either make sure you have trained your own LRP-HAI model (see *Training LRP-HAI* above) or download pretrained weights (see *Pretrained models* above).
2. The main script to launch testing is `experiments/scripts/test_LRP_HAI.sh`. To test your model on the Pascal VOC 2007 test set on GPU-id 0, run `./experiments/scripts/test_LRP_HAI.sh 0 pascal_voc_0712 1 1 0` (see code for more details). If you want to change the exploration-accuracy trade-off parameter, see `experiments/cfgs/LRP-HAI-vgg16.yml`. You may also specify whether you want to visualize LRP-HAI search trajectories here (visualizations are saved in the top folder).
